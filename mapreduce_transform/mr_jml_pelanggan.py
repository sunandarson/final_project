...
class CustomerSum(MRJob):
    ...
    def mapper(self, _, line):
        ...
        if row['id_customer'] != 'id_customer':
            ...
            yield row['date_transaction'][0:4], row['id_customer']
    ...
    def sort(self, key, values):
        data = []
        for date_transaction, customer_total in values:
            data.append((date_transaction, customer_total))
            data.sort()
        for date_transaction, customer_total in data:
            yield date_transaction, customer_total
