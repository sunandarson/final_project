...
if __name__ == '__main__':
    ...
    try:
        ...
        # spark processing trend jumlah transaksi tiap wilayah
        trenddf = sparkdf.groupBy('date_transaction', 'country_customer') \
            .agg((func.sum('amount_transaction'))
                 .alias('jumlah_transact_wilayah')) \
            .toPandas().sort_values(by='date_transaction', ascending=False) \
            .to_csv(path_directory+'dim_country_transact.csv', index=False)\
            .to_sql(f"dim_country_transaction", con=engine_dwh, index=False)
