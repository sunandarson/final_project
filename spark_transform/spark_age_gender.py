...
if __name__ == '__main__':
    ...
    # directory local
        ...
        # spark processing distribusi age dan gender
        dfage = sparkdf.withColumn("age",
                                   F.floor(F.datediff(F.current_date(),
                                                      F.col("birthdate_customer"))/365.25)) \
            .groupBy('age', 'gender_customer')\
            .agg((func.count('age'))
                 .alias('total_gender'))\
            .sort('age').toPandas()\
            .to_csv(path_directory+'dim_age_gender.csv', index=False)\
            .to_sql(f"dim_gender_age",
                    con=engine_dwh, index=False)
